<!DOCTYPE html>
<html>
<head>
	<title>Thêm Phân Công</title>
	<link rel = "icon" type = "image/png" href = "data:image/png;base64,">
	<link rel="stylesheet" type="text/css" href="{{asset('css/boostrap.css')}}"/>
	<link rel="stylesheet" type="text/css" href="{{asset('css/select2.css')}}"/>
	<script src="{{asset('js/jquery.js')}}"></script>
	<style type="text/css">
	   form{
	   	color: black
	   }
	   #div_button a{
	   	color: black
	   }
	   #div_button a:hover{
	  	cursor: pointer;
	  	color: black
	  }
	  button{
	  	background: black;
	  	color: white;
	  	height: 35px;
	  	border-radius: 5px;
	  	width: 20%
	  }
	</style>
</head>
<body>
    @include('menu')
		<div class="container" >
			<div class="col-lg-4" style="border: 1px solid black;width:50%;margin-top: 6%;margin-left: 25%;box-shadow: 15px 20px #F49E9ECD;border-radius: 5px">
				<h1 style="color: black">Tạo phân công mới</h1>
			<form action="{{route('phan_cong.process_insert')}}" method="post" accept-charset="utf-8">
				{{ csrf_field() }}
				<div class="form-group">
				<label>Chọn chuyên ngành</label>
				<select class="form-control" name="ma_chuyen_nganh" id="chuyen_nganh" required="required">
					<option disabled selected>--Chọn Chuyên Ngành--</option>
					@foreach($array_chuyen_nganh as $chuyen_nganh)
						<option value="{{$chuyen_nganh->ma_chuyen_nganh}}">{{$chuyen_nganh->ten_chuyen_nganh}}</option>
					@endforeach
				</select>
				</div>
				<div class="form-group">
					<label>Chọn lớp</label>
					<select class="form-control" name="ma_lop" id="lop" required="required">

					</select>
				</div>
				<div class="form-group" id="div_mon">
					<label>Chọn môn</label>
					<select class="form-control" name="ma_mon" id="mon" required="required">

					</select>
				</div>
				<div class="form-group">
					<label>Chọn Giảng Viên</label>
					<select class="form-control" name="ma_giang_vien" id="giang_vien" disabled="disabled" required="required">

					</select>
				</div>
				<div class="form-group">
					<label>Thời gian định mức</label>
                    <input type="number" name="thoi_gian_dinh_muc_mon" id="thoi_gian_dinh_muc_mon" required="required" pattern="[0-9]+" min=1 >
                    <br>
                    <span style="color:red" id="check_phan_cong" > </span>
				</div>
				<div class="form-group" id="div_button">
					<button type="submit">Thêm</button>
				</div>
			</form>
			</div>
		</div>
		<div style="width: 20%;height: 200px;float: right;margin-top: 20%;margin-right: 7%">
			<p id="check_gv" style="color: red">

			</p>
		</div>
	</div>
</div>
	<script type="text/javascript">
		@if(isset($check[0]))
            alert("Lớp đã được phân công dạy!");
        @elseif(isset($check_thoi_gian))
            $("#check_phan_cong").html("Thời gian định mức phải nhỏ hơn hoặc bằng thời gian định mức môn!");
		@endif
	</script>

	<script src="{{asset('js/select2.js')}}"></script>
	<script type="text/javascript">
		var globalVar;
	   	$(document).ready(function() {

			$("#chuyen_nganh").select2();
			$("#chuyen_nganh").change(function(){
				$('#mon').val(null).trigger('change');
				$('#lop').val(null).trigger('change');
				$('#giang_vien').val(null).trigger('change');
			})
			$('#mon').select2({

				ajax: {
				    url: '{{ route('phan_cong.get_mon_by_chuyen_nganh') }}',
				    dataType: 'json',
				    data: function () {
				        return {
				            ma_chuyen_nganh: $("#chuyen_nganh").val()
				        }
				    },
				    processResults: function (data) {
				      	return {
							results: $.map(data, function (item) {
								return {
									text: item.ten_mon,
									id: item.ma_mon
								}
							})
						};

				    }
				}


			});
			$('#lop').select2({
				ajax: {
				    url: '{{ route('phan_cong.get_lop_by_chuyen_nganh') }}',
				    dataType: 'json',
				    data: function () {
				        return {
				            ma_chuyen_nganh: $("#chuyen_nganh").val()
				        }
				    },
				    processResults: function (data) {
				      	return {
							results: $.map(data, function (item) {
								return {
									text: item.ten_lop,
									id: item.ma_lop
								}
							})
						};
				    }
				},

			});
			$('#giang_vien').select2({
				ajax: {
				    url: '{{ route('phan_cong.get_giang_vien_by_chuyen_nganh') }}',
				    dataType: 'json',
				    data: function () {
				        return {
				            ma_chuyen_nganh: $("#chuyen_nganh").val()
				        }
				    },
				    processResults: function (data) {
				      	return {
							results: $.map(data, function (item) {
								return {
									text: item.ten_giang_vien,
									id: item.ma_giang_vien
								}
							})
						};
				    }
				},

			});
		});
</script>
<script type="text/javascript">
$(document).ready(function(){
	$("select").change(function () {
	   			var count=0;
	   			for (var i = $("select").length - 1; i >= 0; i--) {
	   				if($("select")[i].value!=""){
	   					count++;
	   				}
	   			}
	   			console.log(count);
	   			if(count==3){
	   				$("#giang_vien").removeAttr("disabled");
	   			}
	   			if(count==4){
					var ma_giang_vien = $("#giang_vien").val();
					var ma_mon = $("#mon").val();
					var ma_lop = $("#lop").val();
						$.ajax({

				           type:'GET',

				           url:'{{route('phan_cong.ajax_view_insert')}}',

				           data:{
				           		ma_giang_vien:ma_giang_vien,
				           		ma_mon:ma_mon,
				           		ma_lop:ma_lop
				           	},
				            success:function(data){
				            	globalVar=data;
				              	$("#check_gv").html("");
				              	$("#check_gv").append("<br> Tên giảng viên : "+data[0]);
				              	$("#check_gv").append("<br> Thời gian định mức môn(giờ) : <br>" +data[1]);
				              	$("#check_gv").append("<br> Tổng số giờ làm : "+data[2]+" h");
				              	$("#check_gv").append("<br> Số giờ làm đã xác nhận : "+data[3]+" h");
				              	$("#check_gv").append("<br> Số giờ làm chưa xác nhận : "+data[4]+" h");
				            }
				        });
	   			}
	   		})
});
</script>
</body>
</html>
